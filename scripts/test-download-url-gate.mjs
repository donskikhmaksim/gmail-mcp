...
check("returns a plan", plan1.text.includes("### 📤 План: Ссылки на скачивание"), plan1.text.slice(0, 80));
check("no link was issued", linksIn(plan1.text) === 0, plan1.text.slice(0, 120));
check("preview names the file and the message", /Договор №7\.pdf/.test(plan1.text) && /MSG1/.test(plan1.text), plan1.text.slice(0, 200));
check("preview spells out that the link needs no sign-in", /БЕЗ входа в аккаунт/.test(plan1.text), plan1.text);
check("preview states the lifetime and that it cannot be revoked", /30 мин/.test(plan1.text) && /Отозвать/.test(plan1.text), plan1.text);
