                    ...
                    for (const cc of cell.content ?? []) {
                        for (const pe of cc.paragraph?.elements ?? []) {
                            if (pe.textRun?.content)
                                parts.push(pe.textRun.content.trim());
                        }
                    }
                    return parts.join(" ");
                });
...
function documentEndIndex(doc) {
    const content = doc.body?.content ?? [];
    let end = 1;
    for (const el of content) {
        if (typeof el.endIndex === "number")
            end = el.endIndex;
    }
...
export function registerDocsTools(server, clients) {
    const account = accountField(clients);
    server.registerTool("docs_list", {
        title: "List documents",
        description: "List Google Docs the account can access. Optionally filter by a name substring.",
        inputSchema: {
            account,
    ...
    server.registerTool("docs_read", {
        title: "Read document",
        description: "Read a Google Doc as plain text. Set `raw` to true to get the full structural JSON instead.",
        inputSchema: {
            account,
            documentId: z.string(),
    ...
    server.registerTool("docs_create", {
        ...
        description: "Create a new Google Doc, optionally with initial text. Returns its id.",
        inputSchema: {
            account,
            title: z.string(),
    ...
    server.registerTool("docs_insert_text", {
        ...
        description: "Insert text at a specific character index (1 = very start of the body).",
        inputSchema: {
            account,
            documentId: z.string(),
    ...
    server.registerTool("docs_raw_batch_update", {
        title: "Raw Docs batchUpdate (advanced)",
        description: "Send raw Docs API batchUpdate `requests` (styling, tables, images, etc.). Use only when other tools are not enough.",
        inputSchema: {
            account,
            documentId: z.string(),
