...
export interface ConsentStore {
  ...
  /**
   * НЕОБЯЗАТЕЛЬНЫЙ read-метод: последняя аудит-строка по манифесту (та, где
   * уже заполнен `post_verify_result`, если такая есть — иначе просто самая
   * свежая).
   *
   * Зачем он появился (жалоба Максима 2026-08-14, «подтвердил в вебе, а
   * Claude шлёт запрос по кругу»): когда манифест исполняется ВНЕ этого
   * вызова (веб-хаб `/pending-consents/decide` → `tryAutoExecute` →
   * `executor.execute`), отчёт об исполнении (id письма, пруф post-verify)
   * уходит в браузер человека, а модели доставалось только «уже исполнено
   * через другой канал» — то есть модель не знала, ЧТО именно произошло.
   * Реальный исход мутации лежит ровно в одном месте, доступном серверу, —
   * в `consent_audit.post_verify_result`, который заполняет сам инструмент
   * через `updateConsentAuditOutcome` выше. Эта функция его оттуда достаёт.
   *
   * Метод ОПЦИОНАЛЕН намеренно: не подключён (старый адаптер, офлайн-тест с
   * фейковым стором) ⇒ ветка `already_executed` просто честно пишет
   * «результат не удалось перепроверить», а не падает.
   */
  getAuditByManifest?(
    manifestId: string,
    server: string,
 ...
 *
 * `already_executed` (2026-08-14) — ЧЕТВЁРТЫЙ, отдельный исход: мутация УЖЕ
 * выполнена другим каналом (веб-хаб/Telegram), повторять её НЕЛЬЗЯ, но это и
 * НЕ отказ. Раньше этот случай ехал внутри `refused` с позитивным текстом:
 * защита от двойного исполнения работала (у всех call site'ов есть
 * безусловное «refused → просто верни текст»), но модель получала в `_meta`
 * машинную метку `kind: "refusal"` при тексте «✅ Подтверждено и исполнено» —
 * то есть сервер прямо врал модели про исход, и она повторяла вызов по кругу
 * (жалоба Максима: «подтверждаю за 2 секунды, а он шлёт запрос заново»).
 *
 * ИНВАРИАНТ, КОТОРЫЙ НЕЛЬЗЯ СЛОМАТЬ: в этом исходе НЕТ поля `payload` — и это
 * не забывчивость, а сама защита. Исполнять тулу нечего физически: он не
 * может «случайно» повторить мутацию, потому что данных для неё в исходе нет.
 * `report` — готовый текст (превью + факт исполнения), который тул обязан
 * ретранслировать как ОТЧЁТ ОБ ИСПОЛНЕНИИ (`okVerbatim(..., "execution-report")`),
 * а не как отказ.
 */
...
/**
 * Вырезает query-строку (и фрагмент) из любых `http(s)://` URL внутри текста
 * ошибки — security-checklist.md §6: текст исключения сетевого HTTP-клиента
 * несёт ПОЛНЫЙ URL, включая query, а presigned-ссылка Drive/S3 кладёт токен
 * доступа прямо в query-параметры. Раньше `audit.error` жил только в
 * аудит-таблице (закрытый контур на сервере) и в браузере владельца
 * (веб-хаб) — с исходом `already_executed` он теперь попадает и в ТЕКСТ
 * ОТВЕТА ИНСТРУМЕНТА, то есть в транскрипт модели, откуда токен можно
 * прочитать. Оставляем host+path, вырезаем всё от первого `?`/`#`. Файл
 * сознательно без импортов (переносится в соседние репо) — поэтому простой
 * regex, а не `new URL()` (упал бы на не-URL текстом вокруг совпадения).
 */
function stripUrlQueries(text: string): string {
  return text.replace(/https?:\/\/[^\s"'`)]+/g, (url) => url.replace(/[?#].*$/, ""));
}

// ─────────────── Факт исполнения, случившегося ВНЕ этого вызова ─────────────

/**
 * Сколько раз (включая первую, немедленную попытку) спросить у стора аудит-
 * строку с уже заполненным `post_verify_result`, прежде чем честно сказать
 * «перепроверить не удалось».
 *
 * Зачем ожидание вообще: веб-хаб (`decideOwnConfirm` в http.ts) сначала
 * АТОМАРНО потребляет манифест (`tryAutoExecute` → `consumeManifest`, статус
 * становится DONE) и только ПОТОМ зовёт `executor.execute(...)`, который
 * делает саму мутацию и дописывает пруф в аудит. Значит sync-wait почти
 * всегда видит DONE РАНЬШЕ, чем появился отчёт, — без короткого добора мы бы
 * систематически отдавали модели «результат неизвестен» ровно там, где он
 * через секунду есть.
 */
const EXECUTION_REPORT_POLL_ATTEMPTS = 6;
/** Пауза между попытками добрать отчёт, мс (полсекунды × 5 ≈ 2.5 с максимум). */
const EXECUTION_REPORT_POLL_MS = 500;

/**
 * Достаёт из аудита ФАКТИЧЕСКИЙ исход мутации, которую исполнил кто-то другой.
 * Никогда не бросает: стор без `getAuditByManifest` (или упавший запрос) —
 * это `null`, то есть «не удалось перепроверить», а не ошибка инструмента.
 */
async function fetchExecutionOutcome(
  store: ConsentStore,
  server: string,
  manifestId: string,
): Promise<{ id: string; outcome: string; postVerifyResult?: string | null; error?: string | null } | null> {
  if (!store.getAuditByManifest) return null;
  let last: Awaited<ReturnType<NonNullable<ConsentStore["getAuditByManifest"]>>> = null;
  for (let attempt = 0; attempt < EXECUTION_REPORT_POLL_ATTEMPTS; attempt++) {
    if (attempt > 0) await sleep(EXECUTION_REPORT_POLL_MS);
    try {
      last = await store.getAuditByManifest(manifestId, server);
    } catch {
      return last; // БД моргнула — отдаём что успели, ветка не должна падать
    }
    if (last && (last.postVerifyResult || last.error)) return last;
    // Аудит-строки НЕТ вовсе — ждать пруфа бессмысленно: её пишет тот же
    // `tryAutoExecute` сразу за атомарным consume, ещё ДО самой мутации.
    // Одна добор-попытка — только на микроскопическую гонку «consume уже
    // прошёл, appendConsentAudit ещё нет»; дальше не ждём молча.
    if (!last && attempt >= 1) return null;
  }
  return last;
}

/**
 * Строка «что реально произошло» для тела отчёта. Никогда не молчит: когда
 * пруфа нет — так и написано, без выдумывания успеха (fail-closed на уровне
 * формулировки, инвариант 5 стандарта).
 */
function renderExecutionFact(
  audit: { outcome: string; postVerifyResult?: string | null; error?: string | null } | null,
): string {
  if (!audit) {
    return (
      "**Фактический результат:** перепроверить не удалось — отчёт об исполнении ещё не " +
      "записан в журнал подтверждений (или журнал недоступен). Действие выполнено ровно один " +
      "раз тем каналом, который его подтвердил; что именно получилось — проверь отдельно " +
      "(журнал подтверждений этого сервера)."
    );
  }
  const lines: string[] = [];
  if (audit.postVerifyResult) {
    // `inlineReply` (а не сырая подстановка): пруф собирается сервером, но
    // включает внешние куски (тема письма, имя файла) — они не должны
    // разорвать блок отчёта переводом строки.
    lines.push(`**Фактический результат:** ${inlineReply(audit.postVerifyResult, 600)}`);
  } else if (audit.outcome === "failed") {
    lines.push("**Фактический результат:** исполнение завершилось ОШИБКОЙ (см. ниже).");
  } else {
    lines.push(
      "**Фактический результат:** пруф post-verify пока не записан в журнал — действие " +
        "исполнено, но подтвердить его результат прямо сейчас сервер не может.",
    );
  }
  if (audit.error) lines.push(`**Ошибки при исполнении:** ${inlineReply(stripUrlQueries(audit.error), 300)}`);
  return lines.join("\n\n");
}
  ...
  if (!hasId && !hasReply) {
    ...
    if (syncWaitMs > 0) {
      ...
      if (row && row.status === "DONE") {
        ...
        //
        // ЗАЩИТА ОТ ДВОЙНОГО ИСПОЛНЕНИЯ (не трогать, не «упростить»): веб-хаб
        // (`/pending-consents/decide`) уже реально исполнил мутацию синхронно,
        // внутри своего HTTP-запроса, через `tryAutoExecute`, ДО того, как
        // эта ветка вообще увидела строку DONE. Поэтому отсюда НИКОГДА не
        // возвращается `{kind:"confirmed"}` — это заставило бы вызывающий тул
        // (17 мест в tools/gmail.ts) исполнить payload ЕЩЁ РАЗ: второе письмо,
        // второй ярлык. Ветка ТОЛЬКО ЧИТАЕТ.
        //
        // 2026-08-14: раньше положительный исход ехал в форме
        // `{kind:"refused"}` — защита работала, но в `_meta` модели уходила
        // метка «отказ» при тексте «✅ Подтверждено и исполнено», и модель
        // честно повторяла вызов по кругу. Теперь у него СВОЙ исход
        // `already_executed` — без `payload` (исполнять физически нечего:
        // та же защита, но выраженная типом, а не соглашением) и с честной
        // меткой отчёта об исполнении на стороне тула.
        //
        // Аудит за это исполнение уже записал тот, кто реально исполнил —
        // здесь новую запись не пишем, чтобы не задваивать аудит-лог; только
        // ЧИТАЕМ из него фактический результат для модели.
        const currentHash = await rehash(row.payload);
        const bindingOk = currentHash === row.objectHash;
        const audit = await fetchExecutionOutcome(store, cfg.server, id);
        // Security-review 2026-08-14 (проблема №1, воспроизведено проверяющим
        // на моке gmail: `outcome:"failed"` в аудите всё равно рисовало
        // «✅ Подтверждено и исполнено»): статус манифеста DONE ставится
        // ВНУТРИ `tryAutoExecute` ДО самой мутации (`consumeManifest` в
        // http.ts идёт раньше `executor.execute`) — то есть DONE значит
        // «манифест захвачен», а НЕ «операция удалась». Заголовок и
        // разрешающая формулировка ОБЯЗАНЫ смотреть на фактический исход
        // (`audit.outcome`/`audit.error`/наличие пруфа post-verify), а не
        // только на binding — иначе провалившееся исполнение (Google API
        // 500 и т.п.) рапортуется моделью как успех.
        const executionFailed = audit?.outcome === "failed" || !!audit?.error;
        const hasProof = !!audit?.postVerifyResult;
        let header: string;
        let explain: string;
        if (executionFailed) {
          // Исполнение реально попыталось пройти и провалилось (или бросило
          // исключение — http.ts теперь всегда фиксирует это в аудите как
          // `outcome:"failed"`, см. `executeApprovedCandidate`/
          // `decideOwnConfirm`). НИКОГДА не ✅ и НИКОГДА безусловное «повторять
          // не нужно» — манифест уже одноразово потреблён (повтор С НИМ ничего
          // не даст), но был ли реальный эффект — неизвестно, это ДОЛЖНО быть
          // проверено вручную, а не принято на веру.
          header = "⚠️ Подтверждено, но исполнение НЕ подтверждено";
          explain =
            "Решение по этому плану уже принято, но при исполнении через другой канал " +
            "(веб-подтверждение или Telegram) произошла ОШИБКА (см. «Фактический результат» ниже) " +
            "— результат НЕ подтверждён, действие могло не выполниться вовсе или выполниться частично. " +
            "Этот манифест уже одноразово потреблён, повторный вызов с ним ничего не исполнит. " +
            "ПРОВЕРЬ ВРУЧНУЮ (отдельным чтением), что реально произошло, прежде чем считать действие " +
            "выполненным; если оно действительно не выполнилось и всё ещё нужно — построй НОВЫЙ план.";
        } else if (!bindingOk) {
          header = "⚠️ Подтверждено, но состояние изменилось";
          explain =
            "Действие было подтверждено и, по данным журнала подтверждений, исполнено через другой " +
            "канал, но состояние окружения с момента построения плана успело измениться — фактический " +
            "результат мог не совпасть с тем, что показано в плане выше. Повторно вызывать инструмент " +
            "с этим планом не нужно (манифест уже одноразово потреблён); при сомнении проверь результат " +
            "отдельным чтением.";
        } else if (hasProof) {
          header = "✅ Подтверждено и исполнено";
          explain =
            "Решение по этому плану уже принято, и действие УЖЕ ИСПОЛНЕНО через другой канал " +
            "(веб-подтверждение или Telegram), пока шло ожидание ответа. Мутация выполнена ровно " +
            "один раз и подтверждена независимым post-verify. Повторно вызывать этот инструмент с " +
            "этим планом НЕ нужно — это создало бы дубликат.";
        } else {
          header = "⚠️ Подтверждено, но результат не удалось перепроверить";
          explain =
            "Решение по этому плану уже принято, и, по имеющимся данным, действие исполнено через " +
            "другой канал (веб-подтверждение или Telegram) ровно один раз — но независимый пруф " +
            "post-verify пока не записан в журнал подтверждений (или журнал недоступен), поэтому сервер " +
            "не может подтвердить фактический результат. Повторно вызывать инструмент с этим планом НЕ " +
            "нужно (манифест уже одноразово потреблён); при сомнении в реальном результате проверь " +
            "отдельным чтением.";
        }
        const body = `${previewBody}\n\n${explain}\n\n${renderExecutionFact(audit)}`;
        return {
          kind: "already_executed",
          manifestId: id,
          report: renderConsentBlock(header, body),
