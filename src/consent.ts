...
function renderExecutionFact(
  ...
  if (audit.error) lines.push(`**Ошибки при исполнении:** ${inlineReply(audit.error, 300)}`);
  return lines.join("\n\n");
}
      ...
      if (row && row.status === "DONE") {
        ...
        const explain = bindingOk
          ? "Решение по этому плану уже принято, и действие УЖЕ ИСПОЛНЕНО через другой канал " +
            ...
            "один раз. Повторно вызывать этот инструмент с этим планом НЕ нужно — это создало бы дубликат."
          : "Действие было подтверждено и УЖЕ ИСПОЛНЕНО через другой канал, но состояние окружения с " +
            "момента построения плана успело измениться — фактический результат мог не совпасть с тем, " +
            "что показано в плане выше. Повторно вызывать инструмент НЕ нужно; при сомнении проверь " +
            "результат отдельным чтением.";
