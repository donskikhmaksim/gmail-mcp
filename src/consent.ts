...
export interface RequireConsentParams<T = unknown> {
  ...
  /** Пересчитывает objectHash из ТЕКУЩЕГО состояния (binding). */
  rehash: (payload: unknown) => string | Promise<string>;
  store: ConsentStore;
  cfg: ConsentConfig;
}
...
// Матчинг — по нормализованным ТОКЕНАМ (не подстрокам: «конечно» не должно
// ловиться как «не»). Отрицание проверяется ДО утверждения.
...
const AFFIRMATION_TOKENS = new Set([
  ...
  "го", "погнали", "поехали", "поехали", "плюс", "ясно", "жми", "валяй",
  // EN
...
const NEGATION_TOKENS = new Set([
  // RU
  "нет", "не", "неа", "нельзя", "стоп", "отмена", "отмени", "отменить", "отставить",
  "погоди", "подожди", "стой",
  // EN
...
export function classifyReply(
  ...
  // Отрицание — ПЕРВЫМ: «нет, не отправляй» не должно читаться как «отправляй».
  if (NEGATION_PHRASES.has(norm) || tokens.some((t) => NEGATION_TOKENS.has(t))) {
    return "negation";
  }
...
export async function requireConsent<T = unknown>(
  ...
  // (4) Binding: пересчитанный хеш текущего состояния == сохранённому.
