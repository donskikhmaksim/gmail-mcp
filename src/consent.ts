 ...
 * — В ТЕКСТОВОМ ПУТИ: сервер физически НЕ может доказать, что `user_reply` не
 * сочинён самой моделью. Всё, что сервер вернул в тексте, модель уже знает;
 * протокол MCP не даёт серверу сигнала «пришёл новый ход пользователя». Поэтому
 * текстовый гейт ПОВЕДЕНЧЕСКИЙ, а не криптографический: он превращает тихое
 * само-подтверждение в ЯВНУЮ ЛОЖЬ, которая остаётся в аудит-логе.
 *
 * ГДЕ ЭТОТ ПРЕДЕЛ УЖЕ СНЯТ (2026-08-06): если план РЕАЛЬНО ушёл кнопкой в
 * Telegram и по нажатию его есть чем исполнить, текстовое подтверждение для
 * него закрыто СОВСЕМ — см. `isTgButtonOnly` и ветку (3.4) ниже. Там дыра
 * «модель сочиняет согласие за человека» не уменьшена, а устранена: содержание
 * `user_reply` не влияет ни на что, исполняет сервер по нажатию. Компромисс
 * остаётся ровно для планов БЕЗ кнопки (TG-слой выключен или у тула нет
 * авто-исполнителя). Целевая миграция для них: как только claude.ai получит
 * form-mode elicitation — брать подтверждение оттуда вместо `user_reply`, без
 * смены сигнатур.
 *
